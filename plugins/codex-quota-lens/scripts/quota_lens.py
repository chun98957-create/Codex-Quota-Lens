...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Inspect local Codex quota telemetry or run the Quota Lens dashboard"
    )
    parser.add_argument(
        "command",
        nargs="?",
        choices=("snapshot", "serve"),
        default="snapshot",
        help="print a quota snapshot (default) or serve the local dashboard",
    )
    parser.add_argument("--port", type=int, default=4173)
    parser.add_argument("--codex-home", type=Path, default=Path(os.environ.get("CODEX_HOME", Path.home() / ".codex")))
    parser.add_argument("--compact", action="store_true", help="emit compact JSON for snapshot mode")
    ...
    if args.command == "snapshot":
        ...
        return 0 if payload.get("mode") == "live" else 2

    dashboard_dir = Path(__file__).resolve().parent.parent / "assets" / "dashboard"
    if not (dashboard_dir / "index.html").is_file():
        parser.error(f"dashboard assets are missing: {dashboard_dir}")

    AppHandler.store = store
    handler = partial(AppHandler, directory=str(dashboard_dir))
